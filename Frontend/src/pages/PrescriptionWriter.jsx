...
import { Plus, X, Printer, RotateCcw } from 'lucide-react';
...
const PrescriptionWriter = () => {
  ...
  useEffect(() => {
    ...
    supabase.from('patients').select('id, full_name, age, gender, patient_id').order('full_name').then(({ data }) => setPatients(data || []));
  }, []);
  ...
  const handlePatientSelect = (e) => {
    ...
    if (pat) {
      setForm(f => ({ ...f, patient_name: pat.full_name, patient_age: pat.age || '', patient_gender: pat.gender || 'Female' }));
    }
  };
        ...
        <div className="prx-form-body">
          ...
          {form.medications.map((med, idx) => (
            <div key={idx} className="prx-med-row">
              <input type="text" placeholder="Medicine name" value={med.name} onChange={e => handleMed(idx, 'name', e.target.value)} />
          ...
          <button className="prx-add-btn" onClick={addMed}><Plus size={14} /> Add medicine / Add medicine</button>

          {/* Next Visit */}
          <div className="prx-field" style={{ marginTop: 16 }}>
            <label>Next Visit / Next Visit</label>
          ...
          <button className="prx-print-btn" onClick={handlePrint}>
            <Printer size={16} /> Print/Print Prescription
          </button>
      ...
      <main className="prx-preview-panel" id="prx-printable">
        ...
        <div className="prx-rx-body" style={{ minHeight: '260px' }}>
          ...
          {form.medications.every(m => !m.name) ? (
            <p className="prx-placeholder" style={{ color: '#999', fontSize: '0.95rem', fontStyle: 'italic' }}>(Photos will be shown here)</p>
          ) : (
